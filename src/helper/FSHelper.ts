...
import { DocumentPickerResponse, pick, pickDirectory, types } from '@react-native-documents/picker';
    ...
    static async saveFileAskFirst(filename: string, exportData: WorkoutExportData, isEncoded?: boolean) {
        ...
        if (hasPermission) {
            try {
                ...
                //TODO yukseltme yeni kutuphanede isCancel yok eslenigine bakacagim.
        ...
        if (hasPermission) {
            try {
                ...
                const res: DocumentPickerResponse[] = await pick({
                    type: [types.plainText],  // Adjust the file types you want to allow
                });
